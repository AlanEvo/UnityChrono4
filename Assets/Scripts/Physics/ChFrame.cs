...
namespace chrono
{
    ...
    {
        public ChCoordsys<Real> coord = new ChCoordsys<Real>();  //< Rotation and position, as vector+quaternion
        ...
        public ChFrame(ChVector mv,
                        ChQuaternion mq)
        {
            coord = new ChCoordsys<Real>(mv, mq);
        ...
        {
            coord = new ChCoordsys<Real>(mv, ma.Get_A_quaternion());
            Amatrix = new ChMatrix33<Real>(ma);
        }

        /// Construct from a coordsys
        public ChFrame(ChCoordsys<Real> mc)
        {
            coord = new ChCoordsys<Real>(mc);
        ...
        {
            coord = new ChCoordsys<Real>(mv, alpha, mu);
    ...
    public ChCoordsys<Real> GetCoord() { return coord; }
        ...
        /// already normalized!
        public virtual void SetCoord(ChCoordsys<Real> mcoord)
        {
            coord = mcoord;
