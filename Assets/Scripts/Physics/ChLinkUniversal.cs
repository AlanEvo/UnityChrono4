    ...
    public class ChLinkUniversal : ChLink
    {
        ...
        protected virtual void OnDrawGizmos()
        {
            ...
            if (body1 != null) {
                ...
                Utils.DrawHalfEllipse(transform.position, body1.transform.rotation * offset, body1angle, 0.25f * transform.localScale.x, 0.25f * transform.localScale.y, 32, color);
            }
            if (body2 != null) {
                float body2angle = body2.transform.rotation.eulerAngles.x;
                Utils.DrawHalfEllipse(transform.position, body2.transform.rotation * offset2 * offset3, body2angle, 0.25f * transform.localScale.x, 0.25f * transform.localScale.y, 32, color);
            }
        }
        ...
        public ChMatrix GetC() { return m_C.matrix; }
        ...
        public override void IntStateScatterReactions(int off_L, ChVectorDynamic<double> L)
        {
           /* if (!this.IsActive())
            ...
            m_multipliers[2] = L.matrix[off_L + 2];
            m_multipliers[3] = L.matrix[off_L + 3];
                                         ...
                                         double c)
        {
          /*  if (!IsActive())
            ...
            m_cnstr_x.MultiplyTandAdd(R.matrix, L.matrix[off_L + 0] * c);
            m_cnstr_y.MultiplyTandAdd(R.matrix, L.matrix[off_L + 1] * c);
            m_cnstr_z.MultiplyTandAdd(R.matrix, L.matrix[off_L + 2] * c);
            m_cnstr_dot.MultiplyTandAdd(R.matrix, L.matrix[off_L + 3] * c);*/
        }
        ...
        public override void ConstraintsBiLoad_C(double factor = 1, double recovery_clamp = 0.1, bool do_clamp = false)
        {
          /*  if (!IsActive())
            ...
            double cnstr_x_violation = factor * m_C.matrix.GetElement(0, 0);
            double cnstr_y_violation = factor * m_C.matrix.GetElement(1, 0);
            double cnstr_z_violation = factor * m_C.matrix.GetElement(2, 0);
            double cnstr_dot_violation = factor * m_C.matrix.GetElement(3, 0);
        ...
        // Cached matrices
        private ChMatrix33<double> m_u1_tilde = new ChMatrix33<double>(0);
        private ChMatrix33<double> m_v2_tilde = new ChMatrix33<double>(0);
        ...
        private ChMatrixDynamic<double> m_C = new ChMatrixDynamic<double>();                //< current constraint violations
