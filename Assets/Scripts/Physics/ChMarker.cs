...
namespace chrono
{
    ...
    {        
        ...
        private ChCoordsys<double> rest_coord = new ChCoordsys<double>();  //< relative resting position for function=0.

        private ChCoordsys<double> last_rel_coord = new ChCoordsys<double>();     //< These values are set for each marker update, and are
        private ChCoordsys<double> last_rel_coord_dt = new ChCoordsys<double>();  //< used internally to guess if there's some external routine
        ...
        public ChMarker(string name,
             ChBody body,
             ChCoordsys<double> rel_pos,
             ChCoordsys<double> rel_pos_dt,
             ChCoordsys<double> rel_pos_dtdt)
        {
            SetNameString(name);
            Body = body;
        ...
        /// Also, current position becomes the 'resting position' coordinates
        /// for the current time.
        public void Impose_Abs_Coord(ChCoordsys<double> m_coord) {
            ChBody my_body;
            my_body = GetBody();
        ...
        public ChCoordsys<double> GetRest_Coord() { return rest_coord; }
        ...
        public ChCoordsys<double> GetAbsCoord() { return abs_frame.GetCoord(); }

        /// Get the speed of translation and rotation (as a derived ChCoordsys)
        /// of the marker respect to the absolute coordinates.
        public ChCoordsys<double> GetAbsCoord_dt() { return abs_frame.GetCoord_dt(); }

        /// Get the acceleration of translation and rotation (as a derived ChCoordsys)
        /// of the marker respect to the absolute coordinates.
        public ChCoordsys<double> GetAbsCoord_dtdt() { return abs_frame.GetCoord_dtdt(); }
        ...
        public void SetAbsCoord(ChCoordsys<double> newpos) { abs_frame.SetCoord(newpos); }
        /// Set the speed of translation and rotation (as a ChCoordsys) of the marker
        /// respect to the absolute coordinates.
        /// NOTE! inner use only, for the moment.
        public void SetAbsCoord_dt(ChCoordsys<double> newpos_dt) { abs_frame.SetCoord(newpos_dt); }
        /// Set the speed of translation and rotation (as a ChCoordsys) of the marker
        /// respect to the absolute coordinates.
        /// NOTE! inner use only, for the moment.
        public void SetAbsCoord_dtdt(ChCoordsys<double> newpos_dtdt) { abs_frame.SetCoord(newpos_dtdt); }
        ...
        public void UpdateTime(double mytime) {
            ...
            else
            {
                ...
                csys_dt.rot = new ChQuaternion(0, 0, 0, 0);
                csys_dtdt.rot = new ChQuaternion(0, 0, 0, 0);
            }
        ...
        public void UpdatedExternalTime(double prevtime, double mtime) {
            ...
            Coordsys m_rel_pos_dt = new Coordsys();
            Coordsys m_rel_pos_dtdt = new Coordsys();
            ...
            last_time = ChTime;
            last_rel_coord = (Coordsys)FrameMoving.coord;
            last_rel_coord_dt = (Coordsys)FrameMoving.coord_dt;
        }
